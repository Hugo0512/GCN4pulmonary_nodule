...
flag='center1_'
# flag='test_'
flag2='merge_center1_'
flag3='train_center1_'
...
train_sample_number=train_data_np1.shape[0]
...
for K in allK:
    ...
    for index1 in range(row_number):
        for index3 in range(0,index1+1):
            if index1<train_sample_number:
                adjacent_matrix[index1,index3]=0
        temp=None
        if index1>=train_sample_number:
            temp = adjacent_matrix[index1,0:train_sample_number]
        else:
    ...
    txtfilename=os.path.join(saverootdir,flag2+'adjacent_info_'+str(K)+'.txt')
    np.savetxt(os.path.join(saverootdir,txtfilename),adjacent_info,fmt='%.8f')

txtfilename=os.path.join(saverootdir,flag2+'node_feature'+'.txt')
np.savetxt(os.path.join(saverootdir,txtfilename),node_feature,fmt='%.8f')
txtfilename=os.path.join(saverootdir,flag2+'label'+'.txt')
np.savetxt(os.path.join(saverootdir,txtfilename),train_label,fmt='%d')
txtfilename=os.path.join(saverootdir,flag3+'mask'+'.txt')
np.savetxt(os.path.join(saverootdir,txtfilename),train_mask,fmt='%d')
txtfilename=os.path.join(saverootdir,flag+'mask'+'.txt')
